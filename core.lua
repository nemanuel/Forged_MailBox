...
---@param scroll_bar any
function Forged_Mailbox.skin_uipanel_scrollbar(scroll_bar)
    if not m.api or not scroll_bar or not scroll_bar.GetName then
        return
    end

    local name = scroll_bar:GetName()
    if type(name) ~= "string" or name == "" then
        return
    end

    if scroll_bar.SetWidth then
        scroll_bar:SetWidth(16)
    end

    local up = m.api[name .. "ScrollUpButton"]
    if up then
        -- PaperDoll-style scrollbars match the metal/rivet look.
        up:SetNormalTexture("Interface\\PaperDollInfoFrame\\UI-Character-ScrollBar-ScrollUpButton-Up")
        up:SetPushedTexture("Interface\\PaperDollInfoFrame\\UI-Character-ScrollBar-ScrollUpButton-Down")
        up:SetDisabledTexture("Interface\\PaperDollInfoFrame\\UI-Character-ScrollBar-ScrollUpButton-Disabled")
        up:SetHighlightTexture("Interface\\Buttons\\UI-ScrollBar-ScrollUpButton-Highlight")
    end

    local down = m.api[name .. "ScrollDownButton"]
    if down then
        down:SetNormalTexture("Interface\\PaperDollInfoFrame\\UI-Character-ScrollBar-ScrollDownButton-Up")
        down:SetPushedTexture("Interface\\PaperDollInfoFrame\\UI-Character-ScrollBar-ScrollDownButton-Down")
        down:SetDisabledTexture("Interface\\PaperDollInfoFrame\\UI-Character-ScrollBar-ScrollDownButton-Disabled")
        down:SetHighlightTexture("Interface\\Buttons\\UI-ScrollBar-ScrollDownButton-Highlight")
    end

    if scroll_bar.SetThumbTexture then
        scroll_bar:SetThumbTexture("Interface\\PaperDollInfoFrame\\UI-Character-ScrollBar-Thumb")
    end

    local thumb = m.api[name .. "ThumbTexture"]
    if thumb and thumb.SetTexture then
        thumb:SetTexture("Interface\\PaperDollInfoFrame\\UI-Character-ScrollBar-Thumb")
    end

    -- Some clients/UI packs don't expose the default background regions as globals.
    -- Create our own background so the scrollbar is always visible.
    scroll_bar.fmb_skin = scroll_bar.fmb_skin or {}
    local skin = scroll_bar.fmb_skin

    if not skin.scrollbar_top_background and scroll_bar.CreateTexture then
        skin.scrollbar_top_background = scroll_bar:CreateTexture(nil, "BACKGROUND")
    end

    if skin.scrollbar_top_background then
        skin.scrollbar_top_background:SetTexture("Interface\\PaperDollInfoFrame\\UI-Character-ScrollBar")
        skin.scrollbar_top_background:SetTexCoord(0, 0.5, 0, 1)
        skin.scrollbar_top_background:SetVertexColor(1, 1, 1, 1)
        skin.scrollbar_top_background:ClearAllPoints()
        skin.scrollbar_top_background:SetPoint("TOPLEFT", up, "BOTTOMLEFT", -8, 21)
        skin.scrollbar_top_background:SetPoint("BOTTOMRIGHT", down, "TOPRIGHT", 8, 50)
    end

  if not skin.scrollbar_bottom_background and scroll_bar.CreateTexture then
        skin.scrollbar_bottom_background = scroll_bar:CreateTexture(nil, "BACKGROUND")
    end

    if skin.scrollbar_bottom_background then
        skin.scrollbar_bottom_background:SetTexture("Interface\\PaperDollInfoFrame\\UI-Character-ScrollBar")
        skin.scrollbar_bottom_background:SetTexCoord(0.5, 1, 0.5, 1)
        skin.scrollbar_bottom_background:SetVertexColor(1, 1, 1, 1)
        skin.scrollbar_bottom_background:ClearAllPoints()
        skin.scrollbar_bottom_background:SetPoint("TOPLEFT", up, "BOTTOMLEFT", -9, -171)
        skin.scrollbar_bottom_background:SetPoint("BOTTOMRIGHT", down, "TOPRIGHT", 7, -21)
    end

    local highlight = m.api[name .. "Highlight"]
    if highlight and highlight.SetTexture then
        highlight:SetTexture("Interface\\Buttons\\UI-ScrollBar-Highlight")
    end
end
...
Forged_Mailbox.hook = setmetatable({}, {
...
function Forged_Mailbox.hook.MailFrameTab_OnClick(tab)
    ...
    if tab == 3 or tab == 4 then
        ...
        if m.api.Forged_MailboxLedgerFrame then
            m.api.Forged_MailboxLedgerFrame:Hide()
        end
...
function Forged_Mailbox.ensure_savedvars()
    ...
    settings.SentFilters = settings.SentFilters or {
        Money = 1,
        COD = 1,
        Other = 1
    }
    settings.ReceivedFilters = settings.ReceivedFilters or {
        Money = 1,
        COD = 1,
        Other = 1,
        Returned = 1,
...
function Forged_Mailbox:init()
    ...
    -- Register events
    self.update_frame:SetScript("OnEvent", function()
        self[event]()
    end)
    for _, ev in ipairs({"ADDON_LOADED", "PLAYER_LOGIN", "UI_ERROR_MESSAGE", "CURSOR_UPDATE", "BAG_UPDATE", "MAIL_SHOW",
                         "MAIL_CLOSED", "MAIL_SEND_SUCCESS", "MAIL_INBOX_UPDATE"}) do
    ...
    self.TakeInboxMoney, self.TakeInboxItem, self.DeleteInboxItem = m.api.TakeInboxMoney, m.api.TakeInboxItem,
        m.api.DeleteInboxItem
...
function Forged_Mailbox.on_update()
    if not m.api.MailFrame or not m.api.MailFrame:IsVisible() then
        return
    end
...
function Forged_Mailbox.MAIL_SHOW()
    ...
    if not m._mail_ui_loaded then
        ...
        if type(m.inbox_load) == "function" then
            m.inbox_load()
        end
        if type(m.sendmail_load) == "function" then
            m.sendmail_load()
        end
        if m.log and type(m.log.load) == "function" then
            m.log.load()
        end
        if m.ledger and type(m.ledger.load) == "function" then
            m.ledger.load()
        end
    end
...
function Forged_Mailbox.UI_ERROR_MESSAGE()
    ...
    elseif m.sendmail_sending and
        (arg1 == m.api.ERR_MAIL_TO_SELF or arg1 == m.api.ERR_PLAYER_WRONG_FACTION or arg1 ==
            m.api.ERR_MAIL_TARGET_NOT_FOUND or arg1 == m.api.ERR_MAIL_REACHED_CAP) then
        m.sendmail_sending = false
...
function Forged_Mailbox.ADDON_LOADED()
    ...
    if m.debug_enabled then
        m.info(
            "Expected files: date_picker.lua, core.lua, open_mail.lua, send_mail.lua, autocomplete.lua, log.lua, ledger.lua")
    end
    ...
    if m.api.UIPanelWindows["MailFrame"] then
        ...
        m.api.UIPanelWindows["MailFrame"] = {
            area = "left",
            pushable = 1
        }
    end
    ...
    if m.api.UIPanelWindows["FriendsFrame"] then
        ...
        m.api.UIPanelWindows["FriendsFrame"] = {
            area = "left",
            pushable = 2
        }
    end
    ...
    local function call_or_warn(label, fn)
        ...
        if m.api and m.api.DEFAULT_CHAT_FRAME and m.api.DEFAULT_CHAT_FRAME.AddMessage then
            m.api.DEFAULT_CHAT_FRAME:AddMessage(string.format(
