    ...
    if index <= m.api.GetInboxNumItems() then
      local _, _, sender, _, _, _, _, _, _, was_returned = m.api.GetInboxHeaderInfo( index )
        if INBOX_AUCTIONHOUSES[ sender ] then
          m.api[ "Forged_MailboxAuctionIcon" .. i ]:Show()
        else
          m.api[ "Forged_MailboxAuctionIcon" .. i ]:Hide()
        end
...
function Forged_Mailbox.inbox_load()
  ...
  for i = 1, 7 do
    m.api[ "Forged_MailboxAuctionIcon" .. i .. "Texture" ]:SetVertexColor( m.api.NORMAL_FONT_COLOR.r, m.api.NORMAL_FONT_COLOR.g, m.api.NORMAL_FONT_COLOR.b )
...
function Forged_Mailbox.hook.MailFrameTab_OnClick( tab )
  ...
  if tab == 3 then
    ...
    m.api.Forged_MailboxLogFrame:Hide()
  end
...
function Forged_Mailbox.log.load()
  ...
  m.api.Forged_MailboxLogFiltersButton:SetScript( "OnMouseDown", function()
    m.api.Forged_MailboxLogFiltersButtonArrow:SetPoint( "RIGHT", m.pfui_skin_enabled and -4 or -8, -3 )
  end )
  m.api.Forged_MailboxLogFiltersButton:SetScript( "OnMouseUp", function()
    m.api.Forged_MailboxLogFiltersButtonArrow:SetPoint( "RIGHT", m.pfui_skin_enabled and -4 or -8, -1 )
  end )
  ...
  m.api.Forged_MailboxLogStartTime:SetScript( "OnClick", function()
    m.api.Forged_MailboxLogStartTimeText:SetText( "" )
    m[ m.current_log_type .. "_start_time" ] = nil
    m.log.populate( m.current_log_type )
  end )
  m.api.Forged_MailboxLogEndTime:SetScript( "OnClick", function()
    m.api.Forged_MailboxLogEndTimeText:SetText( "" )
  ...
  m.dropdown_filters = m.api.CreateFrame( "Frame", "Forged_MailboxDropDownFilters" )
...
function Forged_Mailbox.log.filter_dropdown()
  if m.dropdown_filters.initialize ~= Forged_Mailbox.log.filters_menu then
    m.api.CloseDropDownMenus()
    m.dropdown_filters.initialize = Forged_Mailbox.log.filters_menu
  end
...
function Forged_Mailbox.log.populate( log_type, index )
  ...
  for i = 1, 10 do
    if log[ index + i ] then
      ...
      m.api[ "Forged_MailboxLogItem" .. i .. "Status" ]:SetTexture( "" )
      if entry.ah then
        m.api[ "Forged_MailboxLogItem" .. i .. "Participant" ]:SetText( "" )
        m.api[ "Forged_MailboxLogItem" .. i .. "Status" ]:SetTexture( "Interface\\Addons\\Forged_Mailbox\\assets\\AH.blp" )
        m.api[ "Forged_MailboxLogItem" .. i .. "Status" ]:SetPoint( "TOPRIGHT", 4, -10 )
      else
        m.api[ "Forged_MailboxLogItem" .. i .. "Participant" ]:SetText( entry.participant )
      end
      if entry.returned then
        m.api[ "Forged_MailboxLogItem" .. i .. "Status" ]:SetTexture( "Interface\\Addons\\Forged_Mailbox\\assets\\RetArrow.blp" )
        local w = m.api[ "Forged_MailboxLogItem" .. i .. "Participant" ]:GetStringWidth()
        m.api[ "Forged_MailboxLogItem" .. i .. "Status" ]:SetPoint( "TOPRIGHT", -w + 4, -9 )
      end
...
function Forged_Mailbox.pfui_skin()
  if m.api.IsAddOnLoaded( "pfUI" ) and m.api.pfUI and m.api.pfUI.api and m.api.pfUI.env and m.api.pfUI.env.C then
    m.api.pfUI:RegisterSkin( "Forged_Mailbox", "vanilla", function()
      -- Don't apply our skin when Mailbox skin is disabled because it doesn't make sense.
      if m.api.pfUI.env.C.disabled and (m.api.pfUI.env.C.disabled[ "skin_Forged_Mailbox" ] == "1" or m.api.pfUI.env.C.disabled[ "skin_Mailbox" ] == "1") then
        return
      end
      ...
      --Inbox
      m.api.pfUI.api.SkinButton( m.api.Forged_MailboxOpenMailButton )
      ...
      m.api.Forged_MailboxLogTitleText:ClearAllPoints()
      m.api.Forged_MailboxLogTitleText:SetPoint( "TOP", m.api.MailFrame.backdrop, "TOP", 0, -10 )

      m.api.pfUI.api.CreateBackdrop( m.api.Forged_MailboxLogScrollFrame )

      m.api.Forged_MailboxLogScrollFrame:SetHeight( 307 )
      m.api.Forged_MailboxLogScrollFrame:ClearAllPoints()
      m.api.Forged_MailboxLogScrollFrame:SetPoint( "TOPLEFT", 23, -90 )
      m.api.Forged_MailboxLogScrollFrame:SetWidth( 299 )
      m.api.Forged_MailboxLogEntriesFrame:ClearAllPoints()
      m.api.Forged_MailboxLogEntriesFrame:SetPoint( "TOPLEFT", 23, -90 )
      m.api.Forged_MailboxLogEntriesFrame:SetWidth( 299 )

      m.api.pfUI.api.StripTextures( m.api.Forged_MailboxLogItem10 )
      m.api.pfUI.api.SkinScrollbar( m.api.Forged_MailboxLogScrollFrameScrollBar )
      m.api.Forged_MailboxLogScrollFrameScrollBar:SetPoint( "TOPLEFT", m.api.Forged_MailboxLogScrollFrame, "TOPRIGHT", 6, -14 )
      m.api.Forged_MailboxLogScrollFrameScrollBar:SetPoint( "BOTTOMLEFT", m.api.Forged_MailboxLogScrollFrame, "BOTTOMRIGHT", 6, 14 )

      m.api.pfUI.api.SkinButton( m.api.Forged_MailboxLogSentButton )
      m.api.pfUI.api.SkinButton( m.api.Forged_MailboxLogReceivedButton )
      m.api.Forged_MailboxLogReceivedButton:ClearAllPoints()
      m.api.Forged_MailboxLogReceivedButton:SetPoint( "RIGHT", m.api.Forged_MailboxLogSentButton, "LEFT", -2 * bpad, 0 )

      m.api.pfUI.api.SkinButton( m.api.Forged_MailboxLogFiltersButton )
      m.api.Forged_MailboxLogFiltersButton:SetPoint( "TOPLEFT", 21, -58 )
      m.api.Forged_MailboxLogFiltersButton:GetFontString():SetFont( m.api.pfUI.font_default, 12 )
      m.api.Forged_MailboxLogFiltersButton:GetFontString():SetPoint( "TOPLEFT", 4, -4.5 )
      m.api.Forged_MailboxLogFiltersButton:SetWidth( 50 )
      m.api.Forged_MailboxLogFiltersButton:SetHeight( 20 )
      m.api.Forged_MailboxLogFiltersButtonArrow:SetPoint( "RIGHT", -4, -1 )
      m.api.Forged_MailboxLogFiltersButtonArrow:SetWidth( 8.5 )
      m.api.Forged_MailboxLogFiltersButtonArrow:SetHeight( 8.5 )
      m.api.Forged_MailboxLogFiltersButtonArrow:SetTexture( m.api.pfUI.media[ "img:down" ] )

      m.api.pfUI.api.StripTextures( m.api.Forged_MailboxLogStartTime )
      m.api.pfUI.api.SkinButton( m.api.Forged_MailboxLogStartTime )
      m.api.pfUI.api.SkinArrowButton( m.api.Forged_MailboxLogStartTimeButton, "down", 16 )
      m.api.Forged_MailboxLogStartTime:SetPoint( "TOPLEFT", 77, -58 )
      m.api.Forged_MailboxLogStartTimeText:SetPoint( "LEFT", 5, 0 )
      m.api.Forged_MailboxLogStartTimeButton:SetPoint( "LEFT", m.api.Forged_MailboxLogStartTime, "RIGHT", -19, 0 )
      m.api.Forged_MailboxLogStartTimeTitle:SetPoint( "TOPLEFT", 1, 13 )
      m.api.Forged_MailboxLogStartTimeTitle:SetFont( m.api.pfUI.font_default, 10 )
      m.api.Forged_MailboxLogStartTimeTitle:SetText( L[ "Period start" ] )

      m.api.pfUI.api.StripTextures( m.api.Forged_MailboxLogEndTime )
      m.api.pfUI.api.SkinButton( m.api.Forged_MailboxLogEndTime )
      m.api.pfUI.api.SkinArrowButton( m.api.Forged_MailboxLogEndTimeButton, "down", 16 )
      m.api.Forged_MailboxLogEndTime:SetPoint( "TOPLEFT", 163, -58 )
      m.api.Forged_MailboxLogEndTimeText:SetPoint( "LEFT", 5, 0 )
      m.api.Forged_MailboxLogEndTimeButton:SetPoint( "LEFT", m.api.Forged_MailboxLogEndTime, "RIGHT", -19, 0 )
      m.api.Forged_MailboxLogEndTimeTitle:SetPoint( "TOPLEFT", 1, 13 )
      m.api.Forged_MailboxLogEndTimeTitle:SetFont( m.api.pfUI.font_default, 10 )
      m.api.Forged_MailboxLogEndTimeTitle:SetText( L[ "Period end" ] )

      m.api.pfUI.api.SkinDropDown( m.api.Forged_MailboxLogPlayersDropDown, nil, nil, nil, true )
      m.api.UIDropDownMenu_SetWidth( 90, m.api.Forged_MailboxLogPlayersDropDown )
      m.api.Forged_MailboxLogPlayersDropDown:SetScale( 1 )
      m.api.Forged_MailboxLogPlayersDropDown:SetPoint( "TOPLEFT", 224, -54 )
      m.api.Forged_MailboxLogPlayersDropDown.backdrop:SetPoint( "TOPLEFT", 26, -4 )
      m.api.Forged_MailboxLogPlayersDropDown.backdrop:SetPoint( "BOTTOMRIGHT", -20, 8 )
      m.api.Forged_MailboxLogPlayersDropDown.button.backdrop:SetWidth( 16 )
      m.api.Forged_MailboxLogPlayersDropDown.button.backdrop:SetHeight( 16 )

      local label = m.api.Forged_MailboxLogFrame:CreateFontString( nil, "BACKGROUND", "GameFontNormal" )
